...
export async function getReports() {
  try {
    ...
    return {
      success: false,
      reports: [],
      error: response.data?.error || "리포트를 불러올 수 없습니다.",
    };
  } catch (error: any) {
    // 네트워크 에러나 타임아웃 등 모든 에러를 조용히 처리
    console.warn(
      "리포트 목록 조회 실패 (정상 동작 계속):",
      error.message || "알 수 없는 오류",
    );

    // 모든 에러 발생 시 빈 결과 반환 (앱 크래시 방지)
    return {
      success: false,
      reports: [],
      error:
...
export async function getSampleGalleryImages(): Promise<{
  success: boolean;
  ...
  try {
    ...
    const response = await axios.get<{
      ...
      images: SampleGalleryImage[];
      error?: string;
    }>(`/api/gallery/sample-images`, {
      validateStatus: (status) => status < 500, // 500 미만은 성공으로 처리
    });

    // 응답이 성공이거나 에러 객체인 경우 처리
    if (response.status >= 200 && response.status < 300) {
      // 백엔드에서 이미지를 성공적으로 가져온 경우
      if (
      ...
      // 백엔드가 실패했지만 fallback 이미지 사용
      console.warn(
        "백엔드에서 샘플 이미지를 가져오지 못했습니다. fallback 이미지를 사용합니다.",
      );
      return getFallbackSampleImages();
    }

    // 4xx 에러인 경우 fallback 이미지 사용
    console.warn("샘플 이미지 API 에러, fallback 이미지 사용");
    return getFallbackSampleImages();
  } catch (error: any) {
    // 네트워크 에러나 타임아웃 등 모든 에러를 조용히 처리
    console.warn(
      "샘플 이미지 가져오기 실패 (fallback 사용):",
      error.message || "알 수 없는 오류",
    );
...
// 백엔드가 없을 때 사용할 fallback 샘플 이미지 (실제 아이들의 그림)
// Unsplash에서 찾은 실제 아이들이 그린 그림 이미지 (CC0 라이선스)
// 상담을 위해 그려진 다양한 주제와 스타일의 아이들의 그림
// 칠판 분필, 종이 크레용/색연필 등 다양한 매체 사용
function getFallbackSampleImages(): {
  success: boolean;
  images: SampleGalleryImage[];
  error?: string;
} {
