...
const SimpleSwiper = dynamic(() => import("@/components/SimpleSwiper"), {
  loading: () => (
    ...
    const loadAnalysisImages = async () => {
      try {
        ...
        if (
          ...
          console.warn(
            "리포트 로드 경고:",
            response?.error || "응답이 없습니다",
          );
          setGalleryItems([]);
          setGalleryError(null); // 에러 메시지는 표시하지 않고 샘플 이미지만 표시
          // 리포트가 없으면 샘플 이미지 로드
          ...
          <div className="relative">
            {loadingGallery ? (
                    ...
                    try {
                      ...
                      if (
                        ...
                        if (itemsWithImages.length > 0) {
                          ...
                          // 리포트가 없으면 샘플 이미지 로드
                          const sampleResponse = await getSampleGalleryImages();
                          if (
                            sampleResponse.success &&
                            sampleResponse.images &&
                            sampleResponse.images.length > 0
                          ) {
                            const sampleItems = sampleResponse.images.map(
                              (img, index) => ({
                                id: `sample-${index}`,
                                image: img.url,
                                title: img.title,
              ...
              <div className="text-center py-12">
                ...
                <Link
                  href="/analysis"
                  className="inline-block mt-3 bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 text-white px-5 py-2 rounded-lg hover:shadow-lg hover:shadow-cyan-500/50 transition-all text-sm"
                >
                  첫 번째 분석 시작하기
                </Link>
              </div>
