...
import { generateChatReport, downloadReportPDF, createReservation, type CreateReservationRequest, getReport, analyzeImage } from "@/lib/api";
import {
  loadFromIndexedDB,
  deleteFromIndexedDB,
} from "@/lib/indexedDB";
...
export default function AnalysisPage() {
  ...
  const getShareUrl = () => {
    ...
    return `${window.location.origin}/analysis?reportId=${reportData.report.id}`;
  };
    ...
    if (paymentSuccess === "true" && checkoutId && step === 1) {
      const restoreAndAnalyze = async () => {
        try {
          ...
          let fileDataStr = sessionStorage.getItem("pendingAnalysis");
          let fileData: any = null;

          if (fileDataStr) {
            try {
              fileData = JSON.parse(fileDataStr);
              console.log("[AnalysisPage] sessionStorage에서 파일 데이터 로드");
            } catch (e) {
              console.error("[AnalysisPage] sessionStorage 파싱 오류:", e);
            }
          }

          // 2. 없으면 IndexedDB에서 확인
          if (!fileData) {
            fileData = await loadFromIndexedDB("pendingAnalysis");
            if (fileData) {
              console.log("[AnalysisPage] IndexedDB에서 파일 데이터 로드");
            }
          }

          if (!fileData) {
            console.error("[AnalysisPage] 저장된 파일 데이터를 찾을 수 없음");
            alert("저장된 파일 정보를 찾을 수 없습니다. 다시 업로드해주세요.");
            return;
          }
          ...
          console.log("[AnalysisPage] 파일 복원 완료:", {
          ...
          if (analysisResult.success === false) {
            const errorMessage = (analysisResult as any).error || "분석에 실패했습니다.";
            throw new Error(errorMessage);
          }

          console.log("[AnalysisPage] 분석 완료:", {
            reportId: analysisResult.report_id,
          });

          setReportData(analysisResult);
          setStep(2); // Chat 단계로 이동

          // 5. 저장된 데이터 삭제
          sessionStorage.removeItem("pendingAnalysis");
          await deleteFromIndexedDB("pendingAnalysis");
          console.log("[AnalysisPage] 임시 저장 데이터 삭제 완료");
        } catch (error: any) {
          console.error("[AnalysisPage] 파일 복원 및 분석 오류:", error);
          alert(
