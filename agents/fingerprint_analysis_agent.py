...
class FingerprintAnalysisAgent:
    """엄지 손가락 지문 패턴을 분석하고 주성향을 파악하는 전문가 에이전트"""
    
    def __init__(self, llm: ChatOpenAI):
        self.agent = Agent(
            role="지문 유형 검사 전문가",
            goal="엄지 손가락의 지문 패턴을 정확히 분석하고 개인의 주된 성격 특성을 파악한다",
            backstory="""당신은 지문 유형 검사 전문가로서 하워드 가드너의 다중지능 이론을 바탕으로 
            지문 패턴을 분석합니다. 엄지 손가락의 지문 패턴은 개인의 주된 성격 특성을 나타내는 중요한 지표입니다.
