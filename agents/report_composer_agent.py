...
class ReportComposerAgent:
    """모든 분석 결과를 종합하여 상담용 리포트를 작성하는 에이전트"""
    
    def __init__(self, llm: ChatOpenAI):
        self.agent = Agent(
            role="상담 리포트 작성 전문가",
            goal="모든 분석 결과를 종합하여 상담사가 사용할 수 있는 구조화된 리포트를 작성한다",
            backstory="""당신은 미술 치료 상담 리포트 작성 전문가입니다.
            관찰, 감정 분석, 질문 생성 결과를 종합하여 
            상담사가 사용하기 쉽고, 사용자에게 도움이 되는 리포트를 작성합니다.
            리포트는 객관적이고 구조화되어 있으며, 상담의 시작점을 제공합니다.""",
            verbose=True,
            llm=llm,
    ...
    def create_report_composition_task(
        ...
        return Task(
            description=f"""
            다음 분석 결과들을 종합하여 상담용 리포트를 작성하세요:
            ...
            1. 요약 (간단한 전체 요약)
            2. 관찰 사항 (구조화된 관찰 결과)
            3. 감정적 표현 (감정 언어 분석 결과)
            4. 상담 질문 (생성된 질문들)
            5. 상담 시 고려사항 (상담사가 주의할 점)
            
            참고: 전문가 결론은 별도로 생성되므로 리포트 본문에는 포함하지 않습니다.
            리포트는 객관적이고 전문적이며, 상담의 시작점을 제공해야 합니다.
            """,
            agent=self.agent,
            expected_output="구조화된 상담 리포트 (마크다운 형식)"
        )
